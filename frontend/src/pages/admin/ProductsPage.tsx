...
export function ProductsPage() {
  ...
  const query = useAdminProducts({
    ...
    categoryId: rawCategory > 0 ? rawCategory : undefined,
    active: state === 'activo' ? true : state === 'inactivo' ? false : undefined,
    page: filters.page - 1,
    size: 10,
    sortBy,
    sortDir,
  ...
  const columns: DataColumn<ProductResponse, SortKey>[] = [
    ...
    },
    {
      ...
      sortKey: 'name',
      cell: (row) => (
        <Link className="font-semibold hover:underline" to={`/admin/productos/${row.id}/editar`}>
          {row.name}
        </Link>
      ),
    },
    ...
    { id: 'price', header: 'Precio', sortKey: 'price', cell: (row) => formatCurrency(row.price) },
    {
      ...
      cell: (row) => (
        ...
        >
          {row.stock}
          {row.stock === 0 ? ' · Agotado' : row.isLowStock ? ' · Bajo' : ''}
        </span>
      ),
    },
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex items-center gap-3">
          <Link className="underline" to={`/admin/productos/${row.id}/editar`}>
            Editar
          </Link>
          <Button size="sm" variant="outline" onClick={() => setDeleting(row)}>
            Eliminar
          </Button>
        </div>
      ),
    ...
    <div className="space-y-6">
      ...
      <div className="grid items-end gap-4 lg:grid-cols-4">
        ...
        <Select
          id="product-category"
          label="Categoría"
          value={filters.searchParams.get('categoria') ?? ''}
          onChange={(e) => filters.updateParams({ categoria: e.target.value })}
        >
        ...
        <Select
          ...
          label="Estado"
          value={state}
          onChange={(e) => filters.updateParams({ estado: e.target.value })}
        >
      ...
      <DataTable
        ...
        rowKey={(row) => row.id}
        rowClassName={(row) =>
          row.stock === 0 ? 'bg-red-50/50' : row.isLowStock ? 'bg-amber-50/50' : ''
        }
        pagination={{
          page: filters.page,
          size: 10,
        ...
        onSortChange={(sort) =>
          filters.updateParams({ orden: sort.key, direccion: sort.direction })
        }
