...
export const productSchema = z.object({
  ...
  price: z
    ...
    .max(Number.MAX_SAFE_INTEGER, 'Precio demasiado alto.'),
  stock: z
    .number({ error: 'Ingresa el stock.' })
    .int('El stock debe ser entero.')
    .min(0, 'El stock no puede ser negativo.')
    .max(2147483647, 'Stock demasiado alto.'),
  categoryId: z
  ...
  active: z.boolean(),
  images: z
    .array(z.object({ key: z.string(), url: imageUrlSchema, altText: z.string().optional() }))
...
export const categorySchema = z.object({
  ...
  slug: slug.max(120, 'Máximo 120 caracteres.'),
  description: z.string(),
  imageUrl: z.union([z.literal(''), imageUrlSchema]),
  sortOrder: z
    .number({ error: 'Ingresa un orden.' })
